...
export function ProgressView() {
  const { progress, setProgress, settings, todayKey } = useApp();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <Card style={{ textAlign: 'center' }}>
                ...
                {gw && (
                  <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 5 }}>
                    {latest.weight > gw ? `${(latest.weight - gw).toFixed(1)}kg to goal (${gw}kg)` : 'Goal reached'}
                  </div>
                )}
              </Card>
            )}
            {latest?.waist && (
              <Card style={{ textAlign: 'center' }}>
                ...
                {gc && (
                  <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 5 }}>
                    {latest.waist > gc ? `${(latest.waist - gc).toFixed(1)}cm to goal (${gc}cm)` : 'Goal reached'}
                  </div>
                )}
              </Card>
            )}
            ...
            return (
              <Card>
                ...
                <svg width="100%" viewBox={`0 0 ${W} ${H + 4}`} style={{ overflow: 'visible' }}>
                  ...
                  <text x={px(0)} y={H + 14} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.3)" fontFamily="DM Sans">{weightPts[0].label}</text>
                  {weightPts.length > 1 && (
                    <text x={px(weightPts.length - 1)} y={H + 14} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.3)" fontFamily="DM Sans">{weightPts[weightPts.length - 1].label}</text>
                  )}
                </svg>
              </Card>
            );
      ...
      {progress.length === 0 && (
        <div style={{ textAlign: 'center', padding: '2rem 0', color: 'var(--text3)' }}>
          <div style={{ fontSize: 13 }}>Log your first measurements to start tracking progress.</div>
        </div>
      )}
    </div>
  );
